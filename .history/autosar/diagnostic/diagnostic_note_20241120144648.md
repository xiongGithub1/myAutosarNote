 ...
 * @LastEditTime: 2024-11-20 14:46:48
 * @Description: 
-->
...
- 诊断故障码设置控制 ControlDTCSetting(x85)
...
- 诊断会话控制 DiagnosticSessionControl(0x10)
- 会话模式有默认会话模式(default session)和非默认会话模式(non-defaultsession)，非默认会话模式包含编程会话模式(ProgrammingSession)、拓展诊断会话模式(extendedDiagnosticSession)及其余会话模式。会话模式可以理解为诊断的功能模式，即在不同的会话模式下，能够支持不同的诊断功能，如在默认会话模式下，一般情况下不允许支持写服务(WriteDataByIdentifier0x2E)，也不允许支持请求下载服务
