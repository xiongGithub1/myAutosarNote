 ...
 * @LastEditTime: 2024-11-20 14:54:07
 * @Description: 
-->
...
ECU 在刚上电或者复位之后处于默认会话模式(0x01)，默认会话模式下发送1003可以切换至拓展会话模式(0x03)，拓展会话模式发送1002可以切换至编程会话模式(0x02)，而在默认会话模式不可以直接跳转至编程会话模式，若在式会话模式直接发送1002，此时ECU需要响应RC为0x7E(subFunctionNotSupportedInActiveSession)的负响应，响应内容为7F10 7E。
