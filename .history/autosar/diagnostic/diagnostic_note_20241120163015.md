 ...
 * @LastEditTime: 2024-11-20 16:29:52
 * @Description: 
-->
...
该服务用于诊断仪端告知ECU诊断仪依然在线。该服务通常用于保持ECU处于非默认模式，由于ECU在S3server时间收不到诊断请求的话，ECU将会退回默认会话模式(default session)，所以诊断仪为了保持 ECU 处于非默认模式，需要周期性发送TesterPresent服务指令，周期时间需要小于S3server。
